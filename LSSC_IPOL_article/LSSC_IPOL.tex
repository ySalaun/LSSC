...
\Repeat{convergence {\color{red} Marc: apparemment, dans le code on a une boucle sur params.updateIteration = 1. Est-ce qu'il ne vaudrait mieux pas calculer l'argmin \`a chaque boucle et s'arr\^eter lorsque c'est plus petit qu'une certaine valeur ?}}{
	\For{ j = 1..\dsize}{
...
---INITIALIZATION---\\
{\color{blue} Marc : $\A$ n'est pas d\'efinie, et $\da$ non plus.\\}
...
\If{ normX $> \lambda$ {\color{blue} Marc : pourquoi $>$ et pas $<$ ?}}{
	\textbf{return 0}
}
...
\For{ i = 1..$\dsize$}{
	...
	---VARIABLES UPDATES---\\
	u $\leftarrow$ $G_S^{-1} (\text{sgn}(\clars_j))_{j\in \A}$ {\color{blue} Marc : qu'est-ce que \c ca veut dire ? de quelle taille est u ?}\\
	...
	criticalInd $\leftarrow$ j s.t. ratio[j] =	stepMAX {\color{blue} Marc : criticalInd n'est jamais plus utilis\'e par la suite ??!!}\\
	C $\leftarrow$ $\clars[0]$ {\color{blue} Marc : \c ca ne devrait pas \^etre C = max $\clars[j]$ ?} \\
	...
	\If{ $|\gamma| < 1e^{-15} \ ||\ \gamma = stepMAX2 \ ||\ normX < 1e^{-15} \ ||\ normX - \lambda < 1e^{-15}$}{
		\textbf{break} {\color{blue} Marc : ici on travaille en float, donc je pense qu'on peut mettre $1e^{-6}$}
	}
	\If{ $\gamma = stepMAX$ }{
		DOWNDATE $G_S^{-1}$ \\
		newAtom $\leftarrow$ \textbf{False} \\
		i $\leftarrow$ i-2 {\color{blue} Marc c'est possible que i devienne n\'egatif ? \c Ca risque de poser probl\`eme non ?}
	}
	\Else{
		newAtom $\leftarrow$ \textbf{True}
	}
