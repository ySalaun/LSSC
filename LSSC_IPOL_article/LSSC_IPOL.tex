...
\SetLine
	...
	\If{ newAtom }{
		...
		$G_A$[$i^{th}$ column] $\leftarrow$ $G$[currentInd$^{th}$ line]\\
	...
	stepMAX $\in \mathbb{R}^+_* \leftarrow$ $\min^+(\text{ratio})$ ($\min^+$ is the minimum between strictly positive values only)\\
	criticalInd $\in [1;\dsize] \leftarrow \A[j]$ s.t. ratio[j] =	stepMAX\\
	$C \leftarrow |\clars[\A[1]]| = \max_{j=1..\dsize}(|\clars_j|)$ {\color{blue} Marc : \c ca ne devrait pas \^etre C = max $\clars[j]$ ?}{\color{red} Yohann : c est pareil (apres correction)} \\
	$\gamma \in \mathbb{R}^+_* \leftarrow$ $\min^+\left(\frac{C \pm \clars_j}{1 \pm (G_Au)[j]}\right)_{j \text{ s.t. } \A[j] = 0}$ {\color{blue} Marc : \c ca fait 2 possibilit\'es ou bien 4 avec les $\pm$ ?}{\color{red} Yohann : 4 possibilites}\\
	...
	stepMAX2 $\in \mathbb{R}^+ \leftarrow$ $\min(\frac{b - \sqrt{\Delta}}{a}, C)$ \\
	---FINAL STEP $\&$ BREAK---\\
	...
	\If{ $|\gamma| <1e^{-6} \ ||\ \gamma = stepMAX2 \ ||\ $normPatch $ < 1e^{-6} \ ||\ $normPatch $ - \lambda < 1e^{-6}$}{
		\textbf{break}
	}
	\If{ $\gamma = stepMAX$ }{
		DOWNDATE $G_S^{-1}$ w.r.t criticalInd\\
		$\A[\text{criticalInd}] \leftarrow \ 0$\\
		$\code$[criticalInd] $\leftarrow$ 0\\
		newAtom $\leftarrow$ \textbf{False} \\
		i $\leftarrow$ i-2 {\color{blue} Marc c'est possible que i devienne n\'egatif ? \c Ca risque de poser probl\`eme non ?}{\color{red} Yohann non car quand i=1 stepMAX n'est pas défini (car il vaut $min^+(\{0\}) = \emptyset$) et pour i=2 ou plus on fait i=i-2 puis on réincrémente à la fin de la boucle soit i=i-1}
	}
	\Else{
		newAtom $\leftarrow$ \textbf{True}
	}
