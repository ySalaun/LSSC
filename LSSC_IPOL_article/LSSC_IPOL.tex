...
\SetLine
\textbf{Input} :input dictionary \dict = [\textbf{d}$^1$, ... , \textbf{d}$^\dsize$] $\in \mathbb{R}^{\psize \times \dsize}$, \\ \textbf{B} = [\textbf{b}$^1$, ... , \textbf{b}$^\dsize$] $ \in \mathbb{R}^{\dsize \times \dsize}$,  \textbf{C} = [\textbf{c}$^1$, ... , \textbf{c}$^\dsize$]$ \in \mathbb{R}^{\psize \times \dsize}$\\
